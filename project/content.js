...
async function generateResponse(prompt, wordLength, wordMode) {
  try {
    ...
    // Check if enhancer is on - using querySelector within popup
    const enhanceBtn = popup.querySelector('#enhance-btn');
    const isEnhancerOn = enhanceBtn && enhanceBtn.classList.contains('on');
    
    if (isEnhancerOn) {
      // Use simple enhancement prompt
      fullPrompt = `Please enhance this text to make it clearer and more professional: ${prompt}`;
      console.log('Enhancer mode ON - using simple prompt');
    } else {
      // Use the full AI prompt from prompt.js
      const { AIPrompt, About, Rules } = await import(chrome.runtime.getURL('prompt.js'));
      fullPrompt = `${AIPrompt}\n${About}\n${Rules}\n\nUser: ${prompt}`;
      console.log('Enhancer mode OFF - using yoga assistant prompt');
    }

    console.log('Using prompt:', fullPrompt); // Debug log
...
function createPopup() {
    ...
    <div class="ai-assistant-controls">
      <div class="select-container">
        <select id="wordLength">
          ...
          <option value="40-50">100-200 Words</option>
        </select>
        <button id="enhance-btn" class="enhance-btn off">
          <span>Enhancer</span>
        </button>
      </div>
  ...
  // Toggle enhance button with logging
  enhanceBtn.addEventListener('click', () => {
    enhanceBtn.classList.toggle('on');
    enhanceBtn.classList.toggle('off');
    console.log('Enhancer button state changed:', enhanceBtn.classList.contains('on') ? 'ON' : 'OFF');
  });
...
// Add styles for toggle button and enhance button
const style = document.createElement('style');
style.textContent = `
  .enhance-btn {
    background: #ccc; /* Default off state color */
    border: none;
    border-radius: 20px; /* Rounded edges for toggle effect */
    padding: 8px 16px;
    color: white;
    display: flex;
    align-items: center;
  ...
  .enhance-btn.on {
    background: linear-gradient(45deg, #6dd5ed, #2193b0); /* On state color */
  }
  ...
  .enhance-btn.on::after {
    right: calc(100% - 24px); /* Move the circle to the left for on state */
  }
